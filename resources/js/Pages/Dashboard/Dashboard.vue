    ...
    <AppLayout>
        <div class="space-y-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <Link href="/transfers/create"
                        class="px-4 py-2.5 bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white rounded-xl text-sm font-medium transition-all duration-200">+
                        Transfer Money</Link>
                    <Link href="/transactions/create"
                        class="px-4 py-2.5 bg-gradient-to-r from-cyan-500 to-indigo-500 hover:from-cyan-400 hover:to-indigo-400 text-slate-950 font-bold rounded-xl text-sm transition-all duration-200 shadow-lg shadow-cyan-500/10">+
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <!-- Net Balance -->
                <div
                    class="relative bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 transition-all duration-300 hover:border-slate-700">
                    <div
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-slate-900/40 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div v-for="account in accounts" :key="account.id"
                                class="p-4 bg-slate-950/50 border border-slate-800 rounded-xl flex items-center justify-between">
                                <div>
                    ...
                    <div class="bg-slate-900/40 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <Link href="/transactions" class="text-xs font-semibold text-cyan-400 hover:underline">View All</Link>
                        </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-slate-800/60 text-sm">
                                    <tr v-for="tx in transactions" :key="tx.id"
                                        ...
                                        <td class="py-3.5 text-slate-400 whitespace-nowrap">{{ tx.transaction_date }}
                                        </td>
                                        <td class="py-3.5">
                                            <span class="font-medium text-white block">Category #{{ tx.category.name
                                                }}</span>
                                            <span class="text-xs text-slate-500 block max-w-[180px] truncate">{{ tx.note
                                                || 'N/A' }}</span>
                                        </td>
